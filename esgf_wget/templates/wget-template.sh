...
HTTP_BA_FILE="$HOME/.wgetparamfile"
...
prepare_basic_auth_configfile() {
  [ ! -r "$HTTP_BA_FILE" ] && {
    read    -p "HTTP Basic Auth Benutzer-Name: " http_ba_user
    read -s -p "HTTP Basic Auth Passwort:      " http_ba_password
    echo -e "user = $http_ba_user\npassword = $http_ba_password\n" > $HTTP_BA_FILE
    chmod 440 $HTTP_BA_FILE
    echo -e "\n\ncreated the file $HTTP_BA_FILE. You may remove it when you finished.\n\
        NOTE: This contains the information you've just entered.\n\
              You can safely remove it when your download is finished.\n\n"
  }
}
...
#parse flags
while getopts 'F:w:BiuUnSpdvqh' OPT; do
    case $OPT in
        F) input_file="$OPTARG";;       #<file> : read input from file instead of the embedded one (use - to read from stdin)
        w) output="$OPTARG";;           #<file> : Write embedded files into a file and exit
        B) basic_auth=1;;               #       : Use Basic HTTP authentication. Only DWD ESGF has a support for this. It is considered *deprecated*
...
(( basic_auth )) && prepare_basic_auth_configfile
...
download() {
    ...
    {% else %}
    wget="wget ${basic_auth:+--auth-no-challenge --config=$HTTP_BA_FILE} ${insecure:+--no-check-certificate} ${quiet:+-q} ${quiet:--v}"
    {% endif %}
    while read line
    do
